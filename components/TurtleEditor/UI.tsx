...
export default function UserInterface(props: any) {
  ...
  const updateDimensions = () => {
    if (editorpanel.current === undefined || c.codeeditorRef.current === null) {
      return;
    }
  ...
  const resize = (e) => {
    ...
    const newLeftWidth = ((leftPanel.offsetWidth + dx) * 100) / parentoffset;
    const newRightWidth = ((rightPanel.offsetWidth - dx) * 100) / parentoffset;
  ...
  useEffect(() => {
    ...
    return () => {
      if (c.graphicspanelRef && c.graphicspanelRef.current) {
        c.graphicspanelRef.current.removeEventListener("wheel", zoomCanvasFnc);
      }
  ...
  return (
    <div
      className={fullscreen ? "turtlewrapper fullscreen" : "turtlewrapper"}
      ref={c.wrapperRef}
    >
      <div className="row">
        <div className="monacoeditor panel relative" ref={editorpanel}>
          <Editor
            // height="90vh"
          ...
          >
            <a
              title="Undo"
              className={undoBool ? "cursor-pointer" : "opacity-50"}
              onClick={() => {
            ...
            <a
              title="Redo"
              className={redoBool ? "cursor-pointer" : "opacity-50"}
              onClick={() => {
