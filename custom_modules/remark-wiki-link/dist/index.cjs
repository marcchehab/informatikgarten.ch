...
var codes_js = require('micromark-util-symbol/codes.js');
var fs = require('fs');
...
function isEndOfLineOrFile(code) {
  return code === codes_js.codes.carriageReturnLineFeed || code === codes_js.codes.carriageReturn || code === codes_js.codes.lineFeed || code === codes_js.codes.eof;
}
/**
 * Token types:
 * - `wikiLink`:
...
function wikiLink() {
  ...
  var startMarker = codes_js.codes.leftSquareBracket;
  var embedStartMarker = codes_js.codes.exclamationMark;
  var endMarker = codes_js.codes.rightSquareBracket;
  ...
  return {
    text: (_text = {}, _defineProperty(_text, codes_js.codes.leftSquareBracket, wikiLinkConstruct), _defineProperty(_text, codes_js.codes.exclamationMark, wikiLinkConstruct), _text)
  };
}
...
function fromMarkdown() {
  ...
  function exitWikiLink(token) {
    ...
    if (isEmbed) {
      console.log("isSupported", target, isSupportedFileFormat(target));
...
var recursiveGetFiles = function recursiveGetFiles(dir) {
  var dirents = fs__default["default"].readdirSync(dir, {
    withFileTypes: true
  });
  var files = dirents.filter(function (dirent) {
    return dirent.isFile();
  }).map(function (dirent) {
    return path__default["default"].join(dir, dirent.name);
  });
...
exports["default"] = remarkWikiLink;
